...
//    }
    ...
    public void clear() {   //clear list
        size = 0;
        first = null;
        last = null;    //if first = null --> no link for next --> chain is destroyed --> garbage collector will clear it from memory
    }
    ...
    public void add(String stringData) {   //method add nodes to our list, public - because will call from another class, void - nothing to return
        if (isEmpty()) {  //if empty - we add first node (first node = last node), else - jump to the end and create new node = last node
            first = new Node(stringData, null, null); //is list is empty we add node to first element, link to next node - null, data - variable stringData of String type
            ...
            Node newNode = new Node(stringData, null, null);   //last node(newNode) don't have link to next = null
    ...
    @Override
    public ListIterator<String> iterator() {  //our class MyLinkedList must have it's own iterator for working with for-each loop
        return new MyListIterator();  // object of class MyIterator
    }
    ...
    private class Node {   //внутренний класс или вложенный (так как узел(нод) связанного списка LinkedList = обьект, он должен иметь класс)
        ...
        public Node(String data, Node next, Node previous) {   //Constructor for creating nodes with data and link to next node
            this.data = data;
            this.next = next;
